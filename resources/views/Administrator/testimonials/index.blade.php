      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <a href="{{route('admin.add.testimonials')}}" class=" btn btn-info">+ Create Testimonial</a>
          ...
          <div class="card-body">
            <table id="example1" class="table table-bordered table-hover">
              ...
              <tbody>
                ...
                <tr>
                  <td>{{$i++}}</td>
                  <td><img src="{{URL::asset('images/testimonial_images').'/'.$testimonial->images}}" alt="" width="60px"
                      height="60px"></td>
                  <td>{{$testimonial->name}}</td>
                  <td>{{ \Illuminate\Support\Str::limit($testimonial->description, 80, '...')}}</td>
                  ...
                  @endphp
                  {{-- <td><span class="badge badge-{{$color}}">{{ucwords($blog->status)}}</span>
                  </td> --}}
                  <td>
                    {{-- <div class="card-body switch"> --}}
                      <div class="switch">
                        <input type="checkbox" name="toogle" value="{{$testimonial->id}}" {{$testimonial->status=='Active' ?
                  ...
                  <td>
                    <a href="{{route('admin.edit.testimonials',$testimonial->id)}}"><i class="fa fa-edit"></i></a>

                    <form style="margin-left: 36px; margin-top: -24px;" action="{{route('admin.delete.testimonials',$testimonial->id)}}" method="POST">

                      @csrf

                      <a href="" data-toggle="tooltip" title="delete" data-id="{{$testimonial->id}}" style="color: red" data-placement="botton"><i class="far fa-trash-alt dltBtn"></i></a>
...
<script>
  $('input[name=toogle]').change(function(){
      ...
      console.log(id);
      $.ajax({
        url:"{{route('admin.testimonials.status_change')}}",
        type:"POST",
        data:{
          _token:'{{csrf_token()}}',
          mode:mode,
